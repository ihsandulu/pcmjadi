		...
		<div class="row">
			<div class="col-xs-12 col-md-12 col-lg-12">
				<div class="panel panel-default">
					...
					?>
						<div class="">
							<?php if(isset($_POST['edit'])){$namabutton='name="change"';$judul="Update Invoice";}else{$namabutton='name="create"';$judul="Create Invoice";}?>	
							...
							<?php if($inv_no!=""){?>
							 ...
							 if($identity_project!=1){?>
                              ...
                              <div class="form-group">
                                ...
                                <div class="col-sm-10" align="left">
									<?php if($inv_ppn==1){$c='checked="checked"';}else{$c='';}?>
                              ...
                              <div class="form-group">
                                ...
                                <div class="col-sm-10" align="left">
									<textarea id="inv_payment" name="inv_payment" class="form-control"><?=$inv_payment;?></textarea>
									<script>
										CKEDITOR.replace('inv_payment');
									</script>
								</div>
                              </div>	
                              ...
                              <div class="form-group">
                                ...
                                <div class="col-sm-10" align="left">
									<textarea id="inv_note" name="inv_note" class="form-control"><?=$inv_note;?></textarea>
									<script>
										CKEDITOR.replace('inv_note');
									</script>
								</div>
                              </div>
							...
							<hr/>
							 ...
							 <div class="form-group">
								...
								<?php if($inv_picture!=""){$user_image="assets/images/inv_picture/".$inv_picture;}else{$user_image="assets/img/user.gif";}?>
							...
							function updatesimpleinv(){
								...
								.done(function(data){
									var customer_id=0,vendor_id=0,project_id=0,inv_no='';
									$.each(data,function(a,b){//alert(a+"="+b);
										if(a=='customer_id'){customer_id=b;}
										if(a=='vendor_id'){vendor_id=b;}
										if(a=='project_id'){project_id=b;}
										if(a=='inv_no'){inv_no=b;}
									});
									$('#formupdate').hide();
									//alert(customer_id+"/"+vendor_id+"/"+project_id+"/"+inv_no);
									$.get("<?=site_url("api/productsimpleinv");?>",{customer_id:customer_id,vendor_id:vendor_id,project_id:project_id,inv_no:inv_no})
									.done(function(data){										
							...
							function tampilproduct(inv_no){
								$.get("<?=site_url("api/tampilproduct");?>",{inv_no:inv_no})
								.done(function(data){									
									$("#productid").val("");
								...
								<table id="dataTableinv" class="table table-condensed table-hover">
									...
									<tbody> 
										...
										<tr>		
											...
											<?php if($identity->identity_project=="1"){
											if($this->session->userdata("position_id")==1||$this->session->userdata("position_id")==2||$this->session->userdata("position_id")==5||$this->session->userdata("position_id")==7){$disproject='';}else{$disproject='disabled="disabled"';}
											?>
											<td>
												...
												$.get("<?=site_url("api/inputproject_invoice");?>",{project_id:a,inv_no:b})
												.done(function(data){
											...
											<td>
												...
												$.get("<?=site_url("api/inputpoc_invoice");?>",{poc_id:a,inv_no:b})
												.done(function(data){
													window.location.href='<?=current_url();?>?message='+data;
												   $("#messageisi").html(data);
												   $("#message").show();
											...
											if($inv->inv_showproduct<=1){
												foreach($in->result() as $in){
													$to+=($in->invproduct_qty*$in->invproduct_price);
												}
											}else{
											...
											<?php if(!isset($_GET['report'])&&$identity->identity_simple!=1){?> 
                                            	<form method="POST" class="" style="padding:0px; margin:2px; float:right;">
												  <a data-toggle="tooltip" title="Payment" target="_blank" href="<?=site_url("invpayment?inv_no=".$inv->inv_no)."&customer_id=".$inv->customer_id."&project_id=".$inv->project_id."&tagihan=".$invoice;?>" class="btn btn-sm btn-primary " name="payment" value="OK">
