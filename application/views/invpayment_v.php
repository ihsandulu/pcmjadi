...
<head>
	...
	if(isset($_GET['tagihan'])){$tagihan="&tagihan=".$this->input->get("tagihan");}else{$tagihan="";}
	?>
</head>
		...
		<div class="row">
			<div class="col-xs-12 col-md-12 col-lg-12">
				<div class="panel panel-default">
					...
					<?php if(isset($_POST['new'])||isset($_POST['edit'])){?>
						<div class="">
							<?php if(isset($_POST['edit'])){$namabutton='name="change1"';$judul="Update Payment";}else{$namabutton='name="create1"';$judul="New Payment";}?>	
							...
							<form action="<?=$currentUrl;?>" class="form-horizontal" method="post" enctype="multipart/form-data">
							  ...
							  <div class="form-group">
								...
								<div class="col-sm-10">
									...
									if($cek>0){$disabled='disabled="disabled"';}else{$disabled='';}?>
										...
										<tr>
											...
											<td style="padding-left:0px; padding-right:0px;">	 
												...
												<?php if(isset($_GET['customer_id'])){$customer_id=$_GET['customer_id'];}else{$customer_id="";}?>
												  <a data-toggle="tooltip" title="List Payment" target="_blank" href="<?=site_url("invpaymentproduct?invpayment_no=".$invpayment->invpayment_no."&inv_no=".$inv_no."&customer_id=".$customer_id."&project_id=".$_GET["project_id"]."&methodpayment_id=".$invpayment->methodpayment_id).$tagihan;?>" class="btn btn-sm btn-info" style="margin:0px;">
												  <span class="fa fa-shopping-bag" style="color:white;"></span>											  </a>
												</form> 	
												<form method="POST" class="col-md-2" style="padding:0px;">
												  <a data-toggle="tooltip" title="Surat Jalan" href="<?=site_url("sjkeluar?inv_no=".$_GET["inv_no"]."&customer_id=".$_GET["customer_id"]."&project_id=".$_GET["project_id"]);?>" class="btn btn-sm btn-default " name="edit" value="OK">
												  <span class="fa fa-truck" data-fa-transform="flip-v" style="color:green;transform: scaleX(-1);"></span>											  </a>
												</form> 	
												<form method="POST" class="col-md-2" style="padding:0px;">
												  <a data-toggle="tooltip" title="Cost" target="_blank" href="<?=site_url("invcost?invpayment_no=".$invpayment->invpayment_no."&inv_no=".$inv_no."&customer_id=".$customer_id."&project_id=".$_GET["project_id"].$tagihan);?>" class="btn btn-sm btn-info" style="margin:0px;">
