...
class sjkeluarproduct_M extends CI_Model {
	...
	{	
		...
		if($this->input->post("delete")=="OK"){
			...
			//insert stok
			$sj=$this->db
			->where("sjkeluarproduct_id",$this->input->post("sjkeluarproduct_id"))
			->get("sjkeluarproduct");
			foreach($sj->result() as $sj){	
				$istok["gudang_barcode"]=$sj->gudang_barcode;
				$istok["product_id"]=$sj->product_id;
				$istok["varian_id"]=$sj->varian_id;
				$istok["stok_amount"]=$sj->sjmasukproduct_qty;
